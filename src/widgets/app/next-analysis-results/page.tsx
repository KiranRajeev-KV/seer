  ...
  return <main style={{ background: colors.canvas, color: colors.ink, minHeight: 400, maxHeight: maxHeight || 700, overflow: 'auto', padding: 16, boxSizing: 'border-box', fontFamily: 'Georgia, serif' }}>
    <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16, borderBottom: `2px solid ${colors.border}`, paddingBottom: 14 }}>
      <div>
        ...
        <h1 style={{ fontSize: 24, margin: '5px 0 3px', letterSpacing: '-0.03em' }}>{data.taskType === 'regression' ? `${pretty(data.targetColumn)} estimate` : `${pretty(data.targetColumn)} classification`}</h1>
    ...
    <section style={panel(colors)}><strong>{quality.description}</strong><p style={{ color: colors.muted, fontSize: 13, margin: '6px 0 0' }}>{data.model.name} was evaluated against {data.baseline.name} on {data.datasetCoverage.testRows} held-out rows.</p></section>
...
function RegressionView({ data, colors }: { data: RegressionResult; colors: Colors }) {
  return <>
    ...
    <section style={panel(colors)}><h2 style={heading}>Model versus baseline</h2><MetricTable rows={[
      ['MAE', number(data.metrics.model.mae), number(data.metrics.baseline.mae), signedPct(data.metrics.improvement.maePercent)],
      ['RMSE', number(data.metrics.model.rmse), number(data.metrics.baseline.rmse), signedPct(data.metrics.improvement.rmsePercent)],
      ['R²', number(data.metrics.model.r2), number(data.metrics.baseline.r2), `${data.metrics.improvement.r2Absolute >= 0 ? '+' : ''}${number(data.metrics.improvement.r2Absolute)}`],
    ]} model={data.model.name} baseline={data.baseline.name} colors={colors} /></section>
    <div style={twoUp}><Scatter title="Actual vs predicted" points={data.charts.actualVsPredicted.map((point) => ({ x: point.actual, y: point.predicted }))} xLabel="Actual" yLabel="Predicted" diagonal colors={colors} /><Scatter title="Residual vs predicted" points={data.charts.residualVsPredicted.map((point) => ({ x: point.predicted, y: point.residual }))} xLabel="Predicted" yLabel="Residual" horizontalZero colors={colors} /></div>
  </>;
}

function ClassificationView({ data, colors }: { data: ClassificationResult; colors: Colors }) {
  return <>
    <section style={panel(colors)}><h2 style={heading}>Classifications</h2><div style={cards}>{data.predictions.map((prediction, index) => <PredictionCard key={index} title={`ROW ${index + 1}`} value={prediction.predictedClass} detail={`${pct(prediction.predictedProbability)} model probability`} input={prediction.input} coverage={prediction.coverage} colors={colors} />)}</div></section>
    <section style={panel(colors)}><h2 style={heading}>Weighted model versus baseline</h2><MetricTable rows={[
      ['Accuracy', pct(data.metrics.model.accuracy), pct(data.metrics.baseline.accuracy), '—'],
      ['Precision', pct(data.metrics.model.precision), pct(data.metrics.baseline.precision), '—'],
      ['Recall', pct(data.metrics.model.recall), pct(data.metrics.baseline.recall), '—'],
      ['F1', pct(data.metrics.model.f1), pct(data.metrics.baseline.f1), signedPct(data.metrics.improvement.f1Percent)],
    ]} model={data.model.name} baseline={data.baseline.name} colors={colors} /></section>
...
function ConfusionMatrix({ chart, colors }: { chart: ClassificationResult['charts']['confusionMatrix']; colors: Colors }) {
  ...
  return <section style={panel(colors)}><h2 style={heading}>Confusion matrix</h2><p style={{ color: colors.muted, fontSize: 12, marginTop: -4 }}>Rows are actual classes; columns are model classifications.</p><div style={{ display: 'grid', gridTemplateColumns: `72px repeat(${chart.labels.length}, minmax(45px, 1fr))`, gap: 3, fontSize: 12, textAlign: 'center' }}><span /><>{chart.labels.map((label) => <strong key={`head-${label}`} style={{ padding: 5, overflow: 'hidden', textOverflow: 'ellipsis' }}>{label}</strong>)}</>{chart.values.map((row, rowIndex) => <div key={chart.labels[rowIndex]} style={{ display: 'contents' }}><strong style={{ alignSelf: 'center', textAlign: 'right', paddingRight: 5 }}>{chart.labels[rowIndex]}</strong>{row.map((count, columnIndex) => <span key={`${rowIndex}-${columnIndex}`} style={{ background: `rgba(14, 165, 233, ${0.12 + 0.72 * count / maximum})`, borderRadius: 5, padding: 9, fontWeight: 800 }}>{count}</span>)}</div>)}</div></section>;
}
...
function Warnings({ warnings, colors }: { warnings: string[]; colors: Colors }) { return warnings.length === 0 ? null : <section style={panel(colors)}><h2 style={heading}>Warnings and limitations</h2>{warnings.map((warning) => <p key={warning} style={{ background: colors.warning, borderRadius: 6, padding: '8px 10px', fontSize: 13, margin: '7px 0 0' }}>{warning}</p>)}</section>; }
function qualityLabel(quality: Quality) { return { useful_signal: { label: 'Useful signal', tone: '#059669', description: 'The model materially outperformed its baseline on this held-out split.' }, weak_signal: { label: 'Weak signal', tone: '#b45309', description: 'The model improved on its baseline, but the signal is modest.' }, no_demonstrated_signal: { label: 'No demonstrated signal', tone: '#dc2626', description: 'The model did not outperform its baseline on this held-out split.' } }[quality]; }
